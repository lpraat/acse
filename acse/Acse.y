...
{
      t_axe_label *l1, *l2;
      l1 = newLabel(program); // set else label
      l2 = newLabel(program); // end label

      // Register where we will store the result, this will later be used to create the expression
      int result = getNewRegister(program);

      if ($3.expression_type == IMMEDIATE) {
         gen_load_immediate(program, $3.value);
      } else {
         gen_addi_instruction(program, $3.value, $3.value, 0);
      }
      // if condition is false go to set else
      gen_beq_instruction(program, l1, 0);

      // set if exp (here condition is true)
      if ($1.expression_type == IMMEDIATE) {
         gen_addi_instruction(program, result, result, $1.value);
      } else {
         gen_addi_instruction(program, result, $1.value, 0);
      }
      // end
      gen_bt_instruction(program, l2, 0);

      assignLabel(program, l1);
      // set else exp (here condition is false)
      if ($5.expression_type == IMMEDIATE) {
         gen_addi_instruction(program, result, result, $5.value);
      } else {
         gen_addi_instruction(program, result, $5.value, 0);
      }

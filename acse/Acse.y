...
assign_statement : IDENTIFIER LSQUARE exp RSQUARE ASSIGN exp
            ...
            | IDENTIFIER ASSIGN
            {
               _for.index = getNewRegister(program);

               _for.l1 = newLabel(program);
               _for.l_start = newLabel(program);
               _for.l_end = newLabel(program);

               gen_bt_instruction(program, _for.l_start, 0);

               assignLabel(program, _for.l1);
            }
            LSQUARE exp FOR IDENTIFIER IN IDENTIFIER RSQUARE
            {
               t_axe_variable *v1, *v2;
               v1 = getVariable(program, $1);
               v2 = getVariable(program, $9);

               if (!(v1->isArray && v2->isArray)) {
                  printf("Error while parsing array comprehension. Are you using arrays?\n");
                  exit(-1);
               }

               // Store array element
               t_axe_expression index, data;
               index.expression_type = REGISTER;
               index.value = _for.index;

               if ($5.expression_type == IMMEDIATE) {
                  data.expression_type = IMMEDIATE;
                  data.value = $5.value;
               } else {
                  data.expression_type = REGISTER;
                  data.value = $5.value;
               }
               ...
               // Increment index
               gen_addi_instruction(program, _for.index, _for.index, 1);

               // Check if finish
               t_axe_variable *v;
               v = getVariable(program, $1);
               int checkReg = getNewRegister(program);
               gen_subi_instruction(program, checkReg, _for.index, v->arraySize);
               gen_beq_instruction(program, _for.l_end, 0);

               assignLabel(program, _for.l_start);

               // Load array el into identifier
               t_axe_expression e_index;
               e_index.expression_type = REGISTER;
               e_index.value = _for.index;
               int arr_el = loadArrayElement(program, $9, e_index);

               int id_var = get_symbol_location(program, $7, 0);
               gen_add_instruction(program, id_var, REG_0, arr_el, CG_DIRECT_ALL);
               // Jump to set
               gen_bt_instruction(program, _for.l1, 0);

               assignLabel(program, _for.l_end);
            }
;

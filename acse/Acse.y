...
%token INTO AS ON REDUCE
%type <reduce_stmt> REDUCE
...
reduce_statement : REDUCE IDENTIFIER INTO IDENTIFIER
                   {
                      ...
                      assignLabel(program, $1.l1);
                   }
                   AS LSQUARE LSQUARE exp RSQUARE RSQUARE ON IDENTIFIER
                   {
                      t_axe_variable *v1;
                      v1 = getVariable(program, $13);
                      if (!v1->isArray) exit(-1);

                      // Store accumulated result into sum
                      int resultReg = get_symbol_location(program, $4, 0);
                      if ($9.expression_type == IMMEDIATE) {
                         gen_addi_instruction(program, resultReg, REG_0, $9.value);
                      } else {
                         gen_add_instruction(program, resultReg, REG_0, $9.value, CG_DIRECT_ALL);
                      }

                      // Increase array index and check if we have finished
                      gen_addi_instruction(program, $1.index, $1.index, 1);
                      int checkReg = getNewRegister(program);
                      gen_subi_instruction(program, checkReg, $1.index, v1->arraySize);
                      gen_beq_instruction(program, $1.l_end, 0);

                      assignLabel(program, $1.l_start);
                      // load array element into elem
                      int elemReg = get_symbol_location(program, $2, 0);
                      t_axe_expression e_index;
                      e_index.expression_type = REGISTER;
                      e_index.value = $1.index;
                      int arrayElem = loadArrayElement(program, $13, e_index);
                      gen_add_instruction(program, elemReg, REG_0, arrayElem, CG_DIRECT_ALL);
                      // Jump before exp to evaluate it
                      gen_bt_instruction(program, $1.l1, 0);

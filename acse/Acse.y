...
map_statement : MAP
                ...
                IDENTIFIER ON IDENTIFIER AS
                {
                   // Check if we finished
                   t_axe_variable *v1;
                   v1 = getVariable(program, $5);
                   ...
                   // Set element equal to array element
                   int elem = get_symbol_location(program, $3, 0);
                   t_axe_expression e_index;
                   e_index.expression_type = REGISTER;
                   e_index.value = $1.index;
                   int arrayEl = loadArrayElement(program, $5, e_index);
                   gen_add_instruction(program, elem, REG_0, arrayEl, CG_DIRECT_ALL);
                }
                LBRACE IDENTIFIER ASSIGN exp
                {
                   // Here exp is evaluated so store array element
                   ...
                   e_index.expression_type = REGISTER;
                   e_index.value = $1.index;
                   storeArrayElement(program, $5, e_index, data);

                   // Increment index for next iter

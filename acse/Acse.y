...
exp: NUMBER      { $$ = create_expression ($1, IMMEDIATE); }
   ...
   | IDENTIFIER ARRAY_EQ IDENTIFIER
   {
      t_axe_variable *v1, *v2;
      v1 = getVariable(program, $1);
      v2 = getVariable(program, $3);

      if (v1->isArray && v2->isArray && v1->arraySize == v2->arraySize) {
         int result = getNewRegister(program); // where we will store the result
         t_axe_label *l1, *l2, *end;
         l1 = newLabel(program);
         l2 = newLabel(program);
         end = newLabel(program);

         // Index to iter in the arrays
         ...
         int e1 = loadArrayElement(program, $1, e_index);
         int e2 = loadArrayElement(program, $3, e_index);

         int checkReg = getNewRegister(program);
         gen_sub_instruction(program, checkReg, e1, e2, CG_DIRECT_ALL);
         // if e1 != e2 than jump to set false
         gen_bne_instruction(program, l2, 0);

         // increase index
         gen_addi_instruction(program, index, index, 1);

         // check out of bounds
         checkReg = getNewRegister(program);
         gen_subi_instruction(program, checkReg, index, v1->arraySize);

         // if index == arraySize exit otherwise keep going
         gen_bne_instruction(program, l1, 0);

         // Here we finished looping and the array are equal, set result to true
         gen_addi_instruction(program, result, REG_0, 1);
         // jump to end
         gen_bt_instruction(program, end, 0);

         // Here is where we jump in case the array are not equal, set result to false

...
%union {
   ...
   t_for_range_statement for_range_stmt;
}
/*=========================================================================
                               TOKENS
=========================================================================*/
...
control_statement : if_statement         { /* does nothing */ }
            ...
            | for_range_statement        { /* does nothing */ }
...
for_range_statement :
                    ...
                    IDENTIFIER
                    {
                       // Initialize var_range to 0
                       createVariable(program, $3, 0, 0, 0, 0);
                    }
                    IN RANGE
                    LPAR exp RPAR
                    {
                       // Range init block
                       // First iter is performed without increasing the variable
                       int exp_register;
                       if ($8.expression_type == IMMEDIATE) {
                          exp_register = gen_load_immediate(program, $8.value);
                       } else {
                          exp_register = $8.value;
                       }

                       int id_register = get_symbol_location(program, $3, 0);

                       int check_register = getNewRegister(program);

                       // If exp - range_var <= 0 exit
                       gen_sub_instruction(program, check_register, exp_register, id_register, CG_DIRECT_ALL);
                    ...
                    code_block
                    {
                        ...
                        // Temp register
                        int check_register = getNewRegister(program);
                        int tmp_id_register = getNewRegister(program);
                        gen_addi_instruction(program, tmp_id_register, id_register, 1);

                        // If exp - (range_var + 1) <= 0 exit
                        gen_sub_instruction(program, check_register, exp_register, tmp_id_register, CG_DIRECT_ALL);
                        gen_ble_instruction(program, $1.label_end, 0);

                        // We didn't exit so we can do one more iter
                        // Increase range_var and jump to the code block
                        gen_addi_instruction(program, id_register, id_register, 1);
